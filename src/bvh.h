...
struct BVH {
    ...
    void build_bvh(int depth) {
        root = get_root_BVH(mesh);
        grow_bvh(root, mesh, depth);
        cout << "Depth: " << root->depth() << endl;
        cout << "Number of nodes: " << root->n_nodes() << endl;
    }

    void save_as_obj(const std::string& filename) {
        saveBVHAsOBJ(filename, root);
    }

    std::pair<float, float> intersect_leaves(const glm::vec3& ray_o, const glm::vec3& ray_d) {
        glm::vec3 intersection = intersect({ray_o, ray_d}, root);
        return std::make_pair(intersection.x, intersection.y);        
    }
